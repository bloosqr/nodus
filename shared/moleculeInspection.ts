...
export function normalizeReactionPrecedent(data: unknown): ReactionPrecedent | null {
  ...
  const similar = (Array.isArray(value.similar) ? value.similar : []).map((entry) => {
    ...
    const neighbors = (Array.isArray(record.neighbors) ? record.neighbors : []).map((item) => {
      const neighbor = asRecord(item);
      if (!neighbor || typeof neighbor.key !== 'string') return null;
      return { key: neighbor.key, distance: numberOr(neighbor.distance, 0), count: numberOr(neighbor.count, 0) };
    }).filter((item): item is ReactionPrecedentNeighbor => item !== null).slice(0, 8);
    return { input: record.input.slice(0, 4000), neighbors };
...
/** The deterministic precedent section; the model never authors it. */
export function formatReactionPrecedents(precedent: ReactionPrecedent): string {
  ...
  for (const entry of precedent.reactions) {
    lines.push(entry.count > 0
      ? `- ${entry.count} exact precedent(s) for \`${entry.input}\`.`
      : `- No exact precedent for \`${entry.input}\`.`);
  }
  for (const entry of precedent.products) {
    lines.push(entry.count > 0
      ? `- Product \`${entry.input}\`: ${entry.count} recorded route(s) to it.`
      : `- Product \`${entry.input}\`: no recorded route.`);
  }
  for (const item of precedent.similar) {
    const withRoutes = item.neighbors.filter((neighbor) => neighbor.count > 0);
    lines.push(item.neighbors.length
      ? `- Similar to \`${item.input}\`: ${item.neighbors.length} nearest known reaction(s), ${withRoutes.length} with recorded precedent.`
      : `- Similar to \`${item.input}\`: no close known reaction.`);
  }
  return `\n${lines.join('\n')}\n`;
}
